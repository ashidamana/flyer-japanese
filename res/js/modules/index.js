...
//    获取每日一句
    function getDailySentence() {
        $.getJSON('api/v1/getDailySentence.php', function (data) {
            if (data.status == 'ok') {
...
//                console.log(content);
//                console.log('远程获取');
                $('textarea[name=daily_sentence]').val(content);
            }
        });
    }
...
//            console.log('本地读取');
            ...
            setDefaultWhenNothing('select', 'pb_time', setTime());
            setDefaultWhenNothing('input', '50_tone', fifty_tone.fifty_tone);
            setDefaultWhenNothing('radio', 'hang_duan', fifty_tone.hang_duan);
            setDefaultWhenNothing('textarea', 'friend', '@所有人 @水-啊叶 @单晨杰 @橘子 @merline @十六个石榴 @铩 @软糖… @小白小小白 @火-°wendyㄘ  @Lilian Qiu @Pauline @金-YUKI @T @玖言。 @Mia～* @圆脸小胖纸 @嗅着墨香去寻你');
            getDailySentence();
        }
...
//    设置默认值
    ...
    function setDefaultWhenNothing(type, name, value) {
        if (type == 'radio') {
            $('input[name=' + name + '][value=' + value + ']').attr('checked', true);
        } else if (type == 'textarea') {
            $('textarea[name=' + name + ']').val(value);
...
//        数据存储localStorage
        layui.data('flyer_japanese', {
            key: 'cn_name'
            , value: isEmpty($.trim(data.cn_name)) ? '小飞鸟' : $.trim(data.cn_name)
        });
        layui.data('flyer_japanese', {
            key: 'jp_name'
            , value: isEmpty($.trim(data.jp_name)) ? '飛鳥メイラン' : $.trim(data.jp_name)
        });
        layui.data('flyer_japanese', {
            key: 'pb_time'
